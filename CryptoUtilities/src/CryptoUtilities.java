...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        if (!m.isZero()) {
            ...
            NaturalNumber digit = n.divide(m);
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.isZero()) {
            ...
            if (p.compareTo(one) > 0) {
                //make a copy of the n for the case when p is odd
                NaturalNumber subOfN = new NaturalNumber2(n);

                //store the remainder after the p is divided by two in
                //order to restore p
                NaturalNumber remainderP = p.divide(two);

                //recursion
                powerMod(n, p, m);

                NaturalNumber subOfN2 = new NaturalNumber2(n);
                //n^p/2 *n^p/2
                n.multiply(subOfN2);

                //when p is odd multiply n by n one more time
                if (remainderP.compareTo(zero) != 0) {
                    n.multiply(subOfN);
            ...
            //n mod by m no matter if p is equal to one or greater than one
            NaturalNumber finalRemainder = n.divide(m);
            n.copyFrom(finalRemainder);
        }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber wSub1 = new NaturalNumber2(w);

        NaturalNumber wSub2 = new NaturalNumber2(w);

        NaturalNumber nSub = new NaturalNumber2(n);

        NaturalNumber two = new NaturalNumber2(2);

        NaturalNumber one = new NaturalNumber2(1);

        powerMod(wSub1, two, n);

        if (wSub1.compareTo(one) == 0) {
            check = true;
        ...
        powerMod(wSub2, nSub, n);
        if (wSub2.compareTo(one) != 0) {
            check = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        NaturalNumber nSub = new NaturalNumber2(n);
        nSub.decrement();
        //selecting 50 " witness" candidates, and only
        // check for the validity of the candidates that satisfy the
        //requirement clause of the "isWitnessToCompositeness" method.
        for (int x = 0; x < 50; x++) {

            NaturalNumber candidates = randomNumber(n);

            if (candidates.compareTo(one) > 0
                    && candidates.compareTo(nSub) < 0) {

                if (isWitnessToCompositeness(candidates, n)) {
                    isPrime = false;
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        //check to see if the given n is prime
        // if it is do nothing, else add one when it's even or add two when is
        // odd, and check the number every time after the number is incremented
        boolean found = isPrime2(n);
